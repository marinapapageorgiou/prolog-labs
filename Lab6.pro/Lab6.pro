...
% bipolarDivisor(N,D) :- fail. 

bipolarDivisor(N,D) :-	N = 1,						%perittos
						D = 1.
						
bipolarDivisor(N,D) :- (N mod 2) =:= 1,				%perittos
						D is N.
						
/*bipolarDivisor(N,D) :- 	(N mod 2) =:= 0,		%artios
													%Diairetaios to N sketo
						(d mod 2) =:= 1,			%diairetis == odd
						(N mod d) =:= 1.			%piliko == odd */
